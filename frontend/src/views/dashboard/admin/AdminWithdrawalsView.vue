...
import { useAdminReviewWithdraw, useAdminWithdrawRequests } from '@/composables/useAdmin';
...
const query = useAdminWithdrawRequests(page, status);
const reviewMutation = useAdminReviewWithdraw();
...
async function handleConfirm() {
  ...
  try {
    ...
    toast.success(confirmTarget.value.action === 'approve' ? 'Penarikan disetujui.' : 'Penarikan ditolak.');
...
const requests = computed(() => query.data.value?.withdraw_requests ?? []);
</script>
...
<template>
  <DashboardShell>
    <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
      <div>
        <h2 class="font-display text-xl font-semibold text-primary-800">Verifikasi Penarikan</h2>
        <p class="text-sm text-primary-500 mt-1">Saldo berkurang hanya setelah permohonan disetujui di sini.</p>
      </div>
    ...
    <EmptyState v-else-if="!requests.length" icon="📤" title="Tidak ada permohonan penarikan" />
    ...
    <ConfirmModal
      ...
      :title="confirmTarget?.action === 'approve' ? 'Setujui Penarikan?' : 'Tolak Penarikan?'"
